...
test('adaptive thresholds tighten under high volatility', () => {
  ...
  expect(tightened.volatility > 150, 'volatility should rise under repeated shocks');
  expect(tightened.healthFactorMax <= SAMPLE.baseHealthFactorMax, 'health factor max should never exceed base after shocks');
  expect(tightened.gapCapBps <= SAMPLE.baseGapCapBps, 'gap cap should not exceed base during volatility');
});
