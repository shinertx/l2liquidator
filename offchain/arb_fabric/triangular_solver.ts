...
export class TriangularSolver {
  ...
  private async buildLoopEdge(params: {
    ...
    if (this.priceGraph) {
      if (
        !this.priceGraph.isFresh(first.pair.config.id) ||
        !this.priceGraph.isFresh(second.pair.config.id) ||
        !this.priceGraph.isFresh(third.pair.config.id)
      ) {
        return null;
      }
      if (
        !this.validDepth(first.pair, first.venue) ||
        !this.validDepth(second.pair, second.venue) ||
        !this.validDepth(third.pair, third.venue)
      ) {
        return null;
      }
    }
    ...
    let startAmount = this.priceGraph?.suggestBaseAmount(first.pair, first.venue.config.id, this.maxSlippageBps)
      ?? first.pair.tradeSizeBase;
    if (this.priceGraph) {
      const point = this.selectDepthPoint(first.pair, first.venue, startAmount);
      if (!point) return null;
      startAmount = point.amountBaseIn;
    }
    ...
    if (this.priceGraph && !this.depthAccepts(second.pair, second.venue, leg1.amountOut)) {
      return null;
    }
    ...
    if (this.priceGraph && !this.depthAccepts(third.pair, third.venue, leg2.amountOut)) {
      return null;
    }
  ...
  private validDepth(pair: PairRuntime, venue: VenueRuntime): boolean {
    if (!this.priceGraph) return true;
    const points = this.priceGraph.getDepthPoints(pair.config.id, venue.config.id);
    if (points.length === 0) return false;
    return points.some((point) => Math.abs(point.slippageBps) <= this.maxSlippageBps);
  }

  private selectDepthPoint(
    pair: PairRuntime,
    venue: VenueRuntime,
    requiredBaseIn: bigint,
  ): DepthPoint | null {
    if (!this.priceGraph) return null;
    const points = this.priceGraph.getDepthPoints(pair.config.id, venue.config.id);
    if (points.length === 0) return null;
    const sorted = [...points].sort((a, b) => a.amountBaseIn < b.amountBaseIn ? -1 : 1);
    for (const point of sorted) {
      if (point.amountBaseIn < requiredBaseIn) continue;
      if (Math.abs(point.slippageBps) > this.maxSlippageBps) continue;
      return point;
    }
    for (let i = sorted.length - 1; i >= 0; i -= 1) {
      const point = sorted[i];
      if (Math.abs(point.slippageBps) <= this.maxSlippageBps) {
        return point;
      }
    }
    return null;
  ...
  private depthAccepts(pair: PairRuntime, venue: VenueRuntime, requiredBaseIn: bigint): boolean {
    if (requiredBaseIn <= 0n) return false;
    if (!this.priceGraph) return true;
    const point = this.selectDepthPoint(pair, venue, requiredBaseIn);
    return !!point && point.amountBaseIn >= requiredBaseIn;
  }
}
