...
import { BridgeBroker } from './bridge_broker';
...
export class InventoryManager {
  ...
  constructor(private readonly fabric: FabricConfig, private readonly bridgeBroker = new BridgeBroker()) {
  ...
  private enqueueBridge(chainId: number, token: `0x${string}`, deficitUsd: number): void {
    ...
    this.bridgeBroker.publish({ chainId, token, deficitUsd, priority });
  }
