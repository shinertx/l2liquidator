...
import { type Candidate, requestIndexerBoost, fetchBorrowerCandidates } from './indexer/aave_indexer';
...
async function runChainAgent(chain: ChainCfg, cfg: AppConfig) {
  ...
  const iterator = defaultAdapter.streamCandidates(cfg)[Symbol.asyncIterator]();
  ...
  const heartbeatTimer = setInterval(async () => {
    ...
    try {
      if (idleMs >= STALL_POLL_INTERVAL_MS) {
        ...
        const fallback = await defaultAdapter.pollCandidatesOnce(cfg, chain, 250);
  ...
  function schedulePolicyRetry(borrower: `0x${string}`, hf: number, hfMax: number, label: string): void {
  ...
  hfMax = adaptive.healthFactorMax;
      try {
        ...
        histogram.candidateHealthFactor.observe({ chain: chain.name, stage: 'hf_read' }, healthFactor);
      } catch (hfErr) {
      ...
      if ((healthFactor ?? 0) >= hfMax && !precommitEligible) {
        ...
        if (healthFactor !== null) {
          schedulePolicyRetry(candidate.borrower, healthFactor, hfMax, source);
        }
  ...
  const drainPolicyRetry = async (): Promise<boolean> => {
    ...
    for (const [key, entry] of policyRetryQueue) {
      ...
      policyRetryQueue.delete(key);
      try {
        const candidates = await fetchBorrowerCandidates(cfg, chain, entry.borrower);
