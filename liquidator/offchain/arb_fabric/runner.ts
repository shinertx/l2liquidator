...
async function main(): Promise<void> {
  ...
  const singleHopSolver = new SingleHopSolver(registry, registry.fabric);
  const triangularSolver = new TriangularSolver(registry, registry.fabric);
  const crossChainSolver = new CrossChainSolver(registry, registry.fabric);
