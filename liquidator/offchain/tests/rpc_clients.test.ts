...
test('evicting clients clears cached HTTP instances', () => {
  resetRpcClients();
  const first = getRealtimeClient(baseChain);
  expectEqual(first.kind, 'http', 'initial client should default to HTTP');

  evictRpcClients(baseChain.id);
  const second = getRealtimeClient(baseChain);
  expectEqual(second.kind, 'http', 'client after eviction should be recreated');
...
test('enabling WebSocket after disable allows retry attempts', () => {
  ...
  expectEqual(realtime.kind, 'http', 'without WebSocket constructor available, fallback remains HTTP');
});
